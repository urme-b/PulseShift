...
def test_label_climatology_is_leak_free():
    ...
    base = pd.DataFrame(
        {"year": [2022, 2023, 2024], "season": ["summer"] * 3, "daytype": ["weekday"] * 3,
         "hour": [8] * 3, "rides_total": [100, 200, 1000]}
    )
...
def test_bootstrap_ci_ordered():
    ...
    lo, hi = bootstrap_ci(y, np.array([0.1, 0.2, 0.8, 0.9]), roc_auc_score, n=50, require_two_classes=True)
    assert lo <= hi


def test_policy_never_recommends_unsafe_hour():
    df = pd.DataFrame(
        {"ts_local": pd.to_datetime(["2024-07-01 08:00", "2024-07-01 09:00", "2024-07-01 10:00"]),
         "hour": [8, 9, 10], "risk": [0.5, 0.1, 0.6], "heat_index_f": [90, 88, 95],
         "aqi": [60, 60, 60], "expected_rides": [100, 100, 100]}
    )
