...
case-crossover and time-series designs precisely to remove it [10, 11] — so our contribution is not the
...
climatology built from the same calendar the model sees — and we can put a number on it: a predictor
restricted to the `season × daytype × hour` calendar that defines the label (the climatology baseline)
already reaches AUROC 0.69, so that much of the discrimination is shared calendar structure rather than
weather skill, and weather supplies only the lift from 0.69 to 0.94. AUROC therefore overstates genuine
skill; we treat the continuous within-day ride ratio (Section 6.3), not the binary AUROC, as the
substantive estimand for the air-quality question, and keep the binary only as a deployment convenience
for the app. But
...
classical attenuation-corrected within-day effect (β / reliability) is −3.3 points. That r is measured at
the *daily* level, however, while the estimate it corrects is *within-day*: intraday CAMS reliability is
unobserved and plausibly lower than daily, so −3.3 is the optimistic end of the bound and holds only if
intraday reliability is at least as high as daily. That is exactly why we also report reliabilities below
it, which bracket the worse-intraday case: at an assumed 0.5 the corrected effect is −4.8 points, and
only at an aggressive 0.3 does it approach −8.0 — still short of the −10.9 between-day estimate and within
the design's detectable range. So even granting that intraday error is worse than daily, exposure error
attenuates the within-day estimate but does not rescue a large effect; ground-station hourly data would
measure the intraday reliability directly and tighten this rather than overturn it.
...
All figures are regenerated by `run_analysis.py` into `paper/figures/`.

- **Figure 1 (`reliability`).** Reliability diagram on the 2024 hold-out: the unweighted served model
  tracks the diagonal, class-weighting leaves the model over-confident, and isotonic recalibration repairs
  the weighted model (Section 6.1).
- **Figure 2 (`roc`).** ROC curves for the season-aware climatology baseline, the class-weighted model,
  and the served model (Section 6.1).
- **Figure 3 (`decision_curve`).** Decision-curve net benefit versus risk threshold for the served model,
  against treat-all and treat-none references (Section 6.1).
- **Figure 4 (`exposure_response`).** Marginal suppression rate by heat index and by AQI; the AQI panel is
  the confounded marginal association that motivates the identification ladder (Section 6.3).
- **Figure 5 (`aqi_identification`).** Forest plot of the AQI effect (ride-ratio points per +50 AQI) under
  between-day versus within-day identification, collapsing toward zero (Section 6.3).
- **Figure 6 (`smoke_event`).** The June 2023 Canadian-wildfire smoke episode: hourly AQI against hourly
  ridership, Washington DC (Section 6.3).
- **Figure 7 (`ram_by_month`).** Recovered rides by month under the safety-constrained time-shift policy
  (Section 6.5).
...
10. Maclure M. The case-crossover design: a method for studying transient effects on the risk of acute
    events. *American Journal of Epidemiology*. 1991;133(2):144–153. doi:10.1093/oxfordjournals.aje.a115853
11. Bhaskaran K, Gasparrini A, Hajat S, Smeeth L, Armstrong B. Time series regression studies in
    environmental epidemiology. *International Journal of Epidemiology*. 2013;42(4):1187–1195.
